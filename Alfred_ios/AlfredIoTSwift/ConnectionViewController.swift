        ...
        if (connected == false)
        {
            ...
            if (certificateId == nil)
            {
                ...
                if (myImages.count > 0) {
                    ...
                    //
                    // Our application uses a PKCS12 that uses a passphrase created when generating
                    // We suggest using a passphrase for additional security
                    // Passphrase is user defined and should be entered below 
                    
                    if let data = try? Data(contentsOf: URL(fileURLWithPath: myImages[0])) {
                        DispatchQueue.main.async {
                            self.logTextView.text = "found identity \(myImages[0]), importing..."
                        }
                        if AWSIoTManager.importIdentity( fromPKCS12Data: data, passPhrase:"XXXXXXXXXXX", certificateId:myImages[0]) {
                            //
